...
sample_nr = 0
#expected_samples = 500 #2000/4                                        # there are 5 frequencies (alpa...gamma) and 4 sensors, if all 4 sensors are used
expected_samples = 2000 #2148/4                                                # this should be 5 x 4 = 20, the frequency is 10 Hz. 2 seconds of data with all
    ...
    while 1:
        try:        
            if blinked == True:
                ...
                tcpClientSocket.send(('lookRightStart').encode())
                blinked = False
                ...
                #time.sleep(0.3)
                #tcpClientSocket.sendall(message)
                tcpClientSocket.send(('lookLeftStart').encode())
...
def on_signal_received(sensor, data):
    ...
    #'timestamp;Pack;O1;O2;T3;T4;timestamp_ms;label\n'
        for i in range(0, len(data)):
            ...
            #all_signals[0] = flist.filter(data[i].O1)*100
            #all_signals[1] = flist.filter(data[i].O2)*100
            #all_signals[2] = flist.filter(data[i].T3)*100
            #all_signals[3] = flist.filter(data[i].T4)*100
            #print(all_signals)
            #all_samples.append(all_signals) 
            #all_signals.append([flist.filter(data[i].O1), flist.filter(data[i].O2), flist.filter(data[i].T3), flist.filter(data[i].T4)])  
            #all_signals_matrix = [flist.filter(data[i].O1), flist.filter(data[i].O2), flist.filter(data[i].T3), flist.filter(data[i].T4)]
            sample_nr += 4
        ...
        #all_samples.append(all_signals)    
        #all_signals.clear
        #print("Size sample: " + str(len(all_samples)))
        #sample_nr += 1
        #print("sample_nr2: " + str(sample_nr) )        
        ...
        if sample_nr >= expected_samples:               # Collected all samples...
            ...
            #TODO: Test if is needed. https://docs.edgeimpulse.com/docs/edge-impulse-studio/processing-blocks/flatten
...
def inference():
    ...
    #interpreter.set_tensor(0, input_samples)
