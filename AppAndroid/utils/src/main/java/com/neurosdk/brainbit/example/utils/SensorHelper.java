...
public class SensorHelper {
    ...
    private void initBlePerm() {
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)  // greater than or equal  12 android
        {
    ...
    public void enabledSensor(final ISensorEvent callback) {
        if (!_started.getAndSet(true)) {
            if (!isPermissionReady()) { // Check permission
                // Request permissions
                _context.registerForActivityResult(new ActivityResultContracts.RequestMultiplePermissions(), new ActivityResultCallback<Map<String, Boolean>>() {
                    @Override
